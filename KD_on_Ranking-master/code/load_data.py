...
#                 self.n_users, self.m_items = train_record.shape[0], train_record.shape[1]
...
#             rate.append(1.0*count[i]/self.m_items)
#
#
#
#         y = []
...
#
#         self.n_users, self.m_items, self.n_valid = 0, 0, 0
...
#         # self.plot_pics(args)
#         print('n_items:', self.m_items, 'n_users:', self.n_users)
#         sum = 0
#         for item, users in self.train_item_list.items():
#             sum += len(users)
#         print("sparsity:", 1.0*sum/self.m_items/self.n_users)
#
#
#
...
#         #     founction??
#         #     '''
#         #     if pos_items[i] >= self.m_items:
#         #         neg_items[i] += self.m_items
#
...
class Data2(Dataset):
    def load_ori_data(self, args,path):
        self.path = path
        ...
        self.m_items = max(self.m_items,train_data['iid'].max())
    ...
    def __init__(self, config,path='./data/gowalla'):
        self.batch_size = config['bpr_batch_size']
        ...
        self.load_ori_data(config,path)
    ...
    @property
    def allPos(self):
        u_clicked_items = self.train_user_list
        return u_clicked_items
